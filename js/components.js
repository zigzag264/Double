...
const Components = {
    ...
    compareNumbers(prediction, actualResult) {
        ...
        comparePailie3Numbers(predResult, actualResult) {
            if (!predResult || !actualResult) return null;
            const pred = predResult.split('');
            const actual = actualResult.split('');
            const exactMatch = predResult === actualResult;
            const positionHits = pred.filter((d, i) => d === actual[i]);
            const predSet = new Set(pred);
            const actualSet = new Set(actual);
            const digitHits = [...predSet].filter(d => actualSet.has(d));
            return { pred, actual, exactMatch, positionHits, positionHitCount: positionHits.length, digitHits, digitHitCount: digitHits.length };
        },

        createPailie3DigitBox(digit, isHit = false, isActual = false) {
            const box = document.createElement('div');
            box.className = 'p3-digit' + (isHit ? ' hit' : '') + (isActual ? ' actual' : '');
            box.textContent = digit;
            return box;
        },

        createPailie3StrategyRow(prediction, isLast = false, actualResult = null) {
            const row = document.createElement('div');
            row.className = 'p3-strategy-row';
            const hitResult = actualResult ? this.comparePailie3Numbers(prediction.result, actualResult) : null;
            const header = document.createElement('div');
            header.className = 'p3-strategy-header';
            let badges = '';
            if (hitResult && hitResult.exactMatch) {
                badges += '<span class="p3-exact-badge">★ 直选命中</span>';
            } else if (hitResult && hitResult.positionHitCount > 0) {
                badges += '<span class="p3-hit-count-badge">' + hitResult.positionHitCount + '位命中</span>';
            }
            header.innerHTML = '<span class="p3-strategy-name">' + prediction.strategy + '</span>' + badges;
            row.appendChild(header);
            const digitsContainer = document.createElement('div');
            digitsContainer.className = 'p3-digits-row';
            prediction.result.split('').forEach(d => {
                const isHit = hitResult ? hitResult.positionHits.includes(d) : false;
                digitsContainer.appendChild(this.createPailie3DigitBox(d, isHit));
            });
            const sep = document.createElement('span');
            sep.className = 'p3-digits-sep';
            sep.textContent = '+';
            digitsContainer.appendChild(sep);
            const desc = document.createElement('span');
            desc.className = 'p3-strategy-desc';
            desc.textContent = prediction.description;
            digitsContainer.appendChild(desc);
            row.appendChild(digitsContainer);
            if (!isLast) {
                const sepLine = document.createElement('div');
                sepLine.className = 'p3-strategy-separator';
                row.appendChild(sepLine);
            }
            return row;
        },

        createPailie3ModelCard(model, actualResult = null) {
            const card = document.createElement('div');
            card.className = 'p3-model-card';
            const headerClass = this.getModelHeaderClass(model.model_name);
            let bestHitCount = 0;
            model.predictions.forEach(prediction => {
                const hit = this.comparePailie3Numbers(prediction.result, actualResult);
                if (hit && (hit.exactMatch || hit.positionHitCount > bestHitCount)) {
                    bestHitCount = hit.positionHitCount;
                }
            });
            const safeModelId = model.model_id.replace(/[^a-zA-Z0-9-_]/g, '-');
            card.innerHTML = '<div class="p3-model-card-header ' + headerClass + '"><div class="p3-model-card-header-left"><div class="model-icon-box"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5a3 3 0 1 0-5.997.125 4 4 0 0 0-2.526 5.77 4 4 0 0 0 .556 6.588A4 4 0 1 0 12 18Z"><path d="M12 5a3 3 0 1 1 5.997.125 4 4 0 0 1 2.526 5.77 4 4 0 0 1-.556 6.588A4 4 0 1 1 12 18Z"/></svg></div><div class="model-name-wrapper"><h3>' + model.model_name + '</h3><div class="model-id"><span>ID: ' + model.model_id + '</span></div></div></div>' + (actualResult && bestHitCount > 0 ? '<div class="model-best-hit-badge"><svg viewBox="0 0 24 24" fill="currentColor"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg><span>最佳 ' + bestHitCount + '位</span></div>' : '') + '</div><div class="p3-model-card-content"><div class="p3-strategies" id="p3strategies-' + safeModelId + '"></div></div>';
            const container = card.querySelector('#p3strategies-' + safeModelId);
            model.predictions.forEach((pred, i) => {
                container.appendChild(this.createPailie3StrategyRow(pred, i === model.predictions.length - 1, actualResult));
            });
            return card;
        }
