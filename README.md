...
基于 **8 个 AI 大模型** 的双色球彩票预测与数据分析展示平台，提供多模型预测对比、5 类 Chart.js 图表分析、历史命中率回溯，以及每日邮件推送。
...
- 🤖 **多 AI 模型预测** — 8 个前沿大模型（DeepSeek / Qwen / Kimi 等）各用 4 种策略生成 4 组预测，结果对比展示
...
4. 依次调用 8 个 AI 模型，各生成 4 组预测
5. 自动验证：6 红球已排序、蓝球非空、4 组无重复、无 5+ 红球重合
6. 创建备份并保存

### 模型配置

当前配置 8 个模型（`generate_ai_prediction.py` 的 `MODELS` 数组），支持任意兼容 OpenAI 格式的模型：
